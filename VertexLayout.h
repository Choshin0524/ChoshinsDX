...
public:
	// when using Resolve<XXX>() -> take XXX to Type
	...
	const Element& ResoloveByIndex(size_t i) const noexcept(!IS_DEBUG)
	{
		return elements[i];
	}
	template<ElementType Type>
	...
	size_t Size() const noexcept(!IS_DEBUG)
	{
		return elements.empty() ? 0u
			: elements.back().GetOffsetAfter();
	}
...
public:
	...
	template<typename T>
	// && to take rvalue for reference
	void SetAttributeByIndex(size_t i, T && val) noexcept(!IS_DEBUG)
	{
		using namespace DirectX;
		const auto& element = layout.ResoloveByIndex(i);
		auto pAttribute = pData + element.GetOffset();
...
private:
	...
	// ... -> dynamic attributes
	// https://www.youtube.com/watch?v=VAa73H5fwwk&list=PLqCJpWy5Fohd3S7ICFXwUomYW0Wv67pDD&index=32&t=1320s
