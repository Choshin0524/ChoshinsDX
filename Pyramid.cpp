	...
	TestObject(gfx, rng, adist, ddist, odist, rdist)
{
	...
	if (!IsStaticInitialized())
	{
		struct Vertex
		{
			dx::XMFLOAT3 pos;
			dx::XMFLOAT3 n;
			std::array<char, 4> color; // using UNORM to format 1 byte char to 4bytes float
			char padding; // padding 4bytes
		};
		...
		{
			v.color = { (char)20, (char)255, (char)255 };
		}
		for (unsigned short i = 0; i < tesselation; i++)
		{
			model.vertices[i * 3].color = { (char)255, (char)0, (char)0 };
		}
		model.vertices.front().color = { (char)255, (char)0, (char)0 };
